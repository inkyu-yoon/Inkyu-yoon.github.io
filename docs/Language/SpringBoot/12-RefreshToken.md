...
사용하던 Mysql에 저장하기에는, 중요한 데이터가 아니라고 판단했고, 조회시 쿼리문을 실행시켜야하고, 주기적으로 데이터가 무효해지는 하는 점을 감안하여 레디스를 사용한다.
...
>
> 만료된 access Token으로 refresh Token을 찾아와서 유효성을 검사해야할 예정이기 때문이다.
>
