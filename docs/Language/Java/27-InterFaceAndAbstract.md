...
먼저, 공통점은 ***인터페이스와 추상클래스***는 ***구현부가 작성되어 있지 않은 추상 메서드를 구현하도록 강제***하는 역할을 하고

추상메서드를 갖고 있기 때문에 ***인스턴스화 시킬 수 없다***는 점이 있다.



차이점은, ***Interface***의 경우 자바 8 이후 추가된 default · static 메서드를 제외한 메서드들은 ***구현부가 작성되어있지 않아야 하고***
...
그리고 변수는 무조건 ***public static final*** 이어야 하기 떄문에, 이는 제어자는 생략해도 자동적으로 추가된다.
...
인터페이스도 마찬가지로 구현하는 클래스에서 추상메서드를 무조건 구해야한다.
